...
class Swatch(object):
	...
	def add_stripe(self, offset, vert_or_horiz, color):
		self.offset = offset
		self.stripe_color = color
		if vert_or_horiz == True:
			stripe = Stripe((self.x + self.offset, self.y), (self.x + self.offset,self.bottom), vert_or_horiz, self.stripe_color, 3)
			self.stripe_array.append(stripe)
		else:
			print "fixme"
	...
	def draw(self):
		...
		for stripe in self.stripe_array:
			stripe.draw()

class Stripe(object):
	def __init__ (self, start_pos, end_pos, vert_or_horiz, color, thickness):
